<?php
	if (isset($_POST['key'])) {
		...
		if ($_POST['key'] == 'getExistingData') {
			...
			$sql = $conn->query("SELECT user_id, user_name, last_name, age, nation, veg, ptype, ftimes, address, email, password, recive_date, code1, code2, code3
				...
				while($data = $sql->fetch_array()) {
					$response .= '
						<tr>
							...
							<td user_id="country_'.$data["user_id"].'">'.$data["veg"].'</td>
							<td user_id="country_'.$data["user_id"].'">'.$data["ptype"].'</td>
							<td user_id="country_'.$data["user_id"].'">'.$data["ftimes"].'</td>
		...
		$veg = $conn->real_escape_string($_POST['veg']);
		$ptype = $conn->real_escape_string($_POST['ptype']);
		$ftimes = $conn->real_escape_string($_POST['ftimes']);
